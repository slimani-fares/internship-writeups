...
class DPTrainingManager(TrainingManager):
    ...
    * Keep track of the spent privacy budget during training, and block
    ...
    def __init__(  # noqa: PLR0913
        ...
        # Variant H2 (exp_04 follow-up): periodic budget check.
        # Run the expensive get_epsilon every K steps instead of every step.
        # K=1 reproduces canonical behaviour; K=infinity reproduces H1.
        self._budget_check_period = 10
    ...
    def _prevent_budget_overspending(self) -> None:
        ...
        Variant H2 (exp_04 follow-up): periodic budget check.
        accountant.step() is still called per training step (cheap
        bookkeeping; preserves accountant history identical to canonical).
        The expensive get_epsilon() check is run only every K steps.
        Trade-off: budget overspend is detected within K steps of the
        actual exhaustion (vs immediately in canonical). For typical
        FL configurations where σ is calibrated to hit the budget at
        end-of-training, this never matters.
        """

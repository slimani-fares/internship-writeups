...
class DPTrainingManager(TrainingManager):
    ...
    * Keep track of the spent privacy budget during training, and block
    ...
    def _prevent_budget_overspending(self) -> None:
        ...
        Variant H1 (exp_04): defer the expensive per-step `get_epsilon`
        call. The accountant `step()` call is preserved (cheap bookkeeping);
        the RDP epsilon computation is deferred to round boundaries where
        `get_privacy_spent` is already called for logging in
        `_training_round`. For any budget that does NOT get exhausted
        mid-round, this is identical to the canonical behaviour. If a
        budget IS exhausted mid-round, detection happens at end of round
        rather than mid-step (acceptable for typical FL configurations).
        """

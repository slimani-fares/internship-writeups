...
class DPTrainingManager(TrainingManager):
    ...
    * Keep track of the spent privacy budget during training, and block
    ...
    def __init__(  # noqa: PLR0913
        ...
        # Variant H3 (exp_04 follow-up): precompute max-allowed steps once
        # per round via binary search over the RDP accountant. Per-step cost
        # collapses to a single integer compare. Mid-round detection is
        # preserved exactly (unlike H1).
    ...
    def _prevent_budget_overspending(self) -> None:
        ...
        Variant H3 (exp_04 follow-up): the maximum number of steps allowed
        in this round was precomputed once at round start via binary
        search over the RDP accountant (see `_compute_max_steps_for_round`).
        Per-step cost is now a single accountant.step() (cheap) plus an
        integer compare. Mid-round detection is preserved EXACTLY.
        """
        if self.accountant is not None and self._dp_states is not None:
            ...
            if (self._max_steps_this_round is not None
                    and self._step_counter_this_round > self._max_steps_this_round):
                # Mirror canonical rollback: remove the just-added step from
                # history, then raise to interrupt training.
    ...
    def _compute_max_steps_for_round(
        ...
        """Binary search for the largest k such that adding k steps with
        these (noise, srate) keeps total epsilon at-or-below budget.

        Implementation note: opacus's RDPAccountant stores history as
        (noise, srate, count) tuples; appending one tuple with count=k is
        equivalent to calling step() k times. Each binary-search probe is
        therefore one get_epsilon call on a history of size O(rounds), not
        O(steps). With ~17 probes (log2(100k)) per round, total per-round
        precompute cost is ~17 cheap epsilon evaluations regardless of the
        per-round step count.
        """
        budget_eps = self._dp_budget[0]
        budget_delta = self._dp_budget[1]
        # Snapshot the real history; restore at the end so the accountant
        # reflects only steps that ACTUALLY occurred.
    ...
    def _training_round(
        ...
        if self.accountant is not None:
            ...
            # Variant H3: precompute max-allowed steps once per round.
